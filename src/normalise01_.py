#coding:utf-8
from smartSV import processDataSV as pd
import sys
import os
from os import path
import numpy as np
from pandas import DataFrame

def re_normalise(info_form, numericalM2D_m01):
    rows, cls = numericalM2D_m01.shape
    for j in range(cls):
        minj= info_form[j][1]
        distanj = info_form[j][2]
        print("min , distance = ", minj, distanj)
        for i in range(rows):
            vl = numericalM2D_m01[i][j]
            numericalM2D_m01[i][j] = (distanj * vl) + minj

    return numericalM2D_m01

def getFileNameInFilePath(fp):
    lstfp = fp.split('/')
    return lstfp[len(lstfp)-1]

def getNewFileName_info(fp):
    fn = getFileNameInFilePath(fp)
    for i in range(4):
        fn.pop()
    fn += "_info_form.csv"
    return fn

def getNewFileName_matrix(fp):
    fn = getFileNameInFilePath(fp)
    for i in range(4):
        fn.pop()
    fn += "_01.csv"
    return fn


# input like: python normalise01.py -filename 
if(os.path.isfile(path.abspath("data/outlierDone/"+sys.argv[1]))):
    if sys.argv[1] != None:
        df = pd.read_csv("data/outlierDone/"+sys.argv[1])
        list_ncs= []
        for item in df.columns:
            if pd.is_numerical_matrix(np.array([df[item]])):
                list_ncs.append(item)
        numM2D = pd.get_matrix_from_index_in_dataframe(df, list_ncs)
        numericalM2DF = DataFrame(data=numM2D, columns=list_ncs)
        info_form, numericalM2DF_01 = pd.transforme_data_01_index(numericalM2DF, list_ncs)
        pd.write_csv(info_form, ['index','minVL','distanceMinMax'],"normalised01/"+getNewFileName_info(sys.argv[1]),';')
        pd.write_csv(numericalM2DF_01, list_ncs, "normalised01/"+sys.argv[1],';')

else:
    if sys.argv[1] != None:
        df = pd.read_csv("data/filled/"+sys.argv[1])
        list_ncs= []
        for item in df.columns:
            if pd.is_numerical_matrix(np.array([df[item]])):
                list_ncs.append(item)
        numM2D = pd.get_matrix_from_index_in_dataframe(df, list_ncs)
        numericalM2DF = DataFrame(data=numM2D, columns=list_ncs)
        info_form, numericalM2DF_01 = pd.transforme_data_01_index(numericalM2DF, list_ncs)
        pd.write_csv(info_form, ['index','minVL','distanceMinMax'],"normalised01/"+getNewFileName_info(sys.argv[1]),';')
        pd.write_csv(numericalM2DF_01, list_ncs, "normalised01/"+sys.argv[1],';')